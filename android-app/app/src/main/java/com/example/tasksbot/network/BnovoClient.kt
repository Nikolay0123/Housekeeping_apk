...
class BnovoClient(
    ...
    companion object {
        ...
        /**
         * Окно GET /bookings вокруг даты уборки (вперёди чуть короче — типичный горизонт планирования).
         * Важно: у API лимит 50 записей на страницу — «дырки» по номерам чаще из‑за неполной пагинации, а не из‑за окна.
         * Если поддержка Bnovo подтвердит отсечку (заданный период не включает заезд раньше `date_from`) и при
         * поздней дате уборки ранние заезды пропадают даже при полном обходе страниц — увеличьте `BOOKINGS_DATE_PAST_DAYS`.
         */
        const val BOOKINGS_DATE_PAST_DAYS = 120
